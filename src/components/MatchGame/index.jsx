...
const MatchGame = () => {
    ...
    //console.log("match-game-component-score", userScore)
    //console.log("activeTabId", activeTabId)
    const filteredResults = imagesList.filter(each => each.category === activeTabId)
    const checkMatchImage = imageUrl => {
      if(activeImageUrl === imageUrl){
        //console.log("Won")
        setTotalScore(totalScore+1)
        const newUrl = imagesList[Math.floor(Math.random() * imagesList.length)].imageUrl
    ...
    return(
        !isGameOver ? (
          <div className="match-game-bg-container">
            <Header time={time} totalScore={totalScore}/>
            <div className="thumbnail-container">
              <img src={activeImageUrl} className="thumbnail-image"/>
            </div>
            <div className="bottom-container">
            <ul className="tabs-ul-container">
                {tabsList.map(eachTab => <TabItems isActive={activeTabId === eachTab.tabId} tabDetails={eachTab} key={eachTab.tabId} changeActiveTabId={changeActiveTabId}/>)}
            </ul>
            <ul className="images-ul-container">
                {filteredResults.map(eachImage => <ImageItem imageDetails={eachImage} key={eachImage.id} checkMatchImage={checkMatchImage}/>)}
            </ul>
            </div>
        </div>
        ) : (
            <div className='game-over-container'>
              ...
              <div className='game-over-image'>
                  <img src='https://assets.ccbp.in/frontend/react-js/match-game-trophy.png' alt='trophy' className='trophy'/>
                  <h1 className='your-score'>YOUR SCORE</h1>
                  <h2>{totalScore}</h2>
                  <button className='resetButton' onClick={handleResetButton}>
                      <img src='https://assets.ccbp.in/frontend/react-js/match-game-play-again-img.png' className='reset-img'/>
                      PLAY AGAIN
                  </button>
              </div>
          </div>
